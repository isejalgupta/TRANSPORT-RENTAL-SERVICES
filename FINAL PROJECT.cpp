...
// Replace the ReadFromFile() method in the Travel class with this improved version:
static void ReadFromFile() {
    ifstream in("travel_records.txt");
    if (!in) {
    ...
    while (getline(in, line)) {
        ...
        // Use the whole line as a unique identifier
    ...
    do {
        ...
        switch (choice) {
            case 1: {
                ...
                obj.FareDetails();
                obj.Display();
                obj.ProcessPayment(); // Save inside ProcessPayment if successful
                break;
            }

            case 2: {
                Car ob;
                ob.FareDetails();  // Optional: display rates before booking
                ob.Accept();
                ob.Display();
                ob.ProcessPayment();
                break;
